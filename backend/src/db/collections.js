...
const collectionColumns = `c.id, c.name, c.owner_user_id as "ownerUserId", c.created_at as "createdAt",
  ...
  coalesce((select json_agg(cs.user_id order by cs.user_id) from collection_shares cs where cs.collection_id = c.id), '[]'::json) as shares`
  ...
  const result = await withClient(client => client.query(
    `insert into collections (id, owner_user_id, name)
     ...
     returning id, name, owner_user_id as "ownerUserId", created_at as "createdAt",
       true as "isOwner", 0 as "documentCount", '[]'::json as shares`,
    [id, userId, name]
  ))
...
export async function addDocumentToCollection({ userId, collectionId, documentId }) {
  const result = await withClient(client => client.query(
    `insert into collection_documents (collection_id, document_id, added_by)
     select c.id, d.id, $1
     from collections c
     join documents d on d.id = $3 and d.user_id = c.owner_user_id
     where c.id = $2 and c.owner_user_id = $1
...
export async function shareCollection({ userId, collectionId, targetUserId }) {
  const result = await withClient(client => client.query(
    `insert into collection_shares (collection_id, user_id)
     ...
     on conflict do nothing
     returning collection_id as "collectionId", user_id as "userId"`,
    [userId, collectionId, targetUserId]
  ))
