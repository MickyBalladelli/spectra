...
create index if not exists query_audit_logs_user_created_idx on query_audit_logs(user_id, created_at desc);
...
create index if not exists search_feedback_user_created_idx on search_feedback(user_id, created_at desc);
...
alter table collection_shares add column if not exists role text not null default 'viewer';
